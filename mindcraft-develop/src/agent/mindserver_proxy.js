...
class MindServerProxy {
    ...
    async connect(name, port) {
        ...
        this.socket.on('apply-settings', (nextSettings) => {
            try {
                setSettings(nextSettings);
                console.log(this.name, 'applied updated settings without restart');
            } catch (error) {
        ...
        this.socket.on('send-message', (data) => {
            try {
                this.agent.respondFunc(data.from, data.message, data.channel || 'api');
            } catch (error) {
...
export function sendVoiceLogToServer(agentName, payload) {
    serverProxy.getSocket().emit('voice-log', {
        agentName,
        ...payload,
    });
}
