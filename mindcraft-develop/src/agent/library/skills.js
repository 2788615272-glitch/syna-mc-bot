...
const synaProbe = createSynaProbeClient();
const synaProbeAffectsActions = settings.syna_probe?.affect_actions === true;

function markCombatTarget(bot, entity, reason = 'combat') {
    try { bot?.emit?.('synaCombatTarget', entity, reason); } catch (_) {}
}

async function waitForPlacedBlock(bot, target_dest, blockType, timeoutMs=1200) {
    const acceptable = new Set([blockType]);
    if (blockType === 'dirt') acceptable.add('grass_block');
    if (blockType === 'redstone') acceptable.add('redstone_wire');

    const start = Date.now();
    while (Date.now() - start <= timeoutMs) {
        const block = bot.blockAt(target_dest);
        if (block && acceptable.has(block.name)) {
            return true;
        }
        if (synaProbe.enabled && synaProbeAffectsActions) {
            await synaProbe.getState(); // probe heartbeat for fresher server-side state; block-specific endpoint can be added later.
        }
...
export async function attackEntity(bot, entity, kill=true) {
    ...
    if (!kill) {
        ...
        markCombatTarget(bot, entity, 'attack');
...
export async function defendSelf(bot, range=9) {
    ...
    while (enemy) {
        ...
        markCombatTarget(bot, enemy, 'self_defense');
...
export async function collectBlock(bot, blockType, num=1, exclude=null) {
    ...
    for (let i=0; i<num; i++) {
        if ((bot.inventory?.emptySlotCount?.() ?? 1) <= 0) {
            try { bot.emit('synaInventoryFull', { reason: 'collectBlock:' + blockType }); } catch (_) {}
            log(bot, 'Inventory is full while collecting ' + blockType + '; LLM should clean inventory once before continuing.');
        }
        ...
        try {
            ...
            if (err.name === 'NoChests') {
                try { bot.emit('synaInventoryFull', { reason: 'collectBlock:' + blockType + ':NoChests' }); } catch (_) {}
...
export async function pickupNearbyItems(bot) {
    ...
    const distance = 8;
    if ((bot.inventory?.emptySlotCount?.() ?? 1) <= 0) {
        try { bot.emit('synaInventoryFull', { reason: 'pickup_nearby_items' }); } catch (_) {}
        log(bot, 'Inventory is full before picking up nearby items; LLM should clean inventory once.');
        return false;
    }
...
export async function breakBlockAt(bot, x, y, z) {
    ...
    if (block.name !== 'air' && block.name !== 'water' && block.name !== 'lava') {
        ...
        try {
            bot.emit('synaBlockBroken', { name: block.name, position: block.position.clone?.() || block.position, time: Date.now() });
        } catch (_) {}
...
export async function placeBlock(bot, blockType, x, y, z, placeOn='bottom', dontCheat=false) {
    ...
    // --- 鏂瑰潡鍚嶁啋鐗╁搧鍚嶆槧灏勶紙澶勭悊 MC 鐗堟湰宸紓鍜屾柟鍧?鐗╁搧鍚嶄笉涓€鑷达級 ---
    const BLOCK_TO_ITEM = {
    ...
    if (BLOCK_TO_ITEM[item_name]) {
        item_name = BLOCK_TO_ITEM[item_name];
    }
    ...
    if (!block_item && bot.game.gameMode === 'creative' && !bot.restrict_to_inventory) {
        const madeItem = mc.makeItem(item_name, 1);
        if (!madeItem) {
            // makeItem 杩斿洖 null 璇存槑鏂瑰潡鍚嶆棤鏁堬紝涓嶈浼?null 缁?setInventorySlot
            log(bot, `Cannot create item "${item_name}" (invalid block/item name for this MC version). Skipping.`);
            console.log(`[BP-DIAG] makeItem returned null for "${item_name}" (original blockType="${blockType}")`);
            return false;
        }
        await bot.creative.setInventorySlot(36, madeItem); // 36 is first hotbar slot
    ...
    try {
        if (item_name.includes('bucket')) {
            ...
            const confirmed = await waitForPlacedBlock(bot, target_dest, blockType);
            if (!confirmed) {
                bot.clearControlStates();
                log(bot, `Placed ${blockType} at ${target_dest}, but placement was not confirmed in time.`);
                return false;
            }
            if (synaProbeAffectsActions) {
                await synaProbe.waitUntilStableOnGround(500);
            }
            bot.setControlState('jump', false);
...
export async function equip(bot, itemName) {
    ...
    // 鏌ユ壘鐗╁搧锛氭敮鎸?mod 鐗╁搧鐨勫绉嶅悕绉版牸寮?    // mineflayer 瀵?Forge mod 鐗╁搧鍙兘鍙瓨 numeric ID 鑰?name 鍥為€€涓?vanilla 鍚?    // 鎵€浠ユ垜浠仛澶氬眰鍖归厤锛氱簿纭悕 鈫?鍚懡鍚嶇┖闂?鈫?nbt customName 鈫?slot type 鍖归厤
    let item = _findItemInInventory(bot, itemName);
    ...
    else if (itemName.includes('helmet') || itemName.includes('_hat') || itemName.includes('_cap') || itemName.includes('_hood')) destination = 'head';
    else if (itemName.includes('chestplate') || itemName.includes('elytra') || itemName.includes('_armor') || itemName.includes('_vest')) destination = 'torso';
    else if (itemName.includes('shield')) destination = 'off-hand';

    try {
        await bot.equip(item, destination);
    } catch (equipErr) {
        // mineflayer equip 鍙兘鍥犱负 mod 鐗╁搧 type 涓嶅尮閰嶈€屽け璐?        // 鍥為€€锛氱敤 raw window click 鎶婄墿鍝佺Щ鍒板搴旇澶囨Ы
        log(bot, `[equip] mineflayer equip failed (${equipErr.message}), trying raw slot swap...`);
        try {
            await _rawEquipToSlot(bot, item, destination);
        } catch (rawErr) {
            log(bot, `[equip] raw equip also failed: ${rawErr.message}`);
            return false;
        }
    }
...
/**
 * 鍦ㄨ儗鍖呬腑鏌ユ壘鐗╁搧锛屾敮鎸?mod 鐗╁搧鐨勫绉嶅悕绉版牸寮? */
function _findItemInInventory(bot, itemName) {
    const slots = bot.inventory.slots;
    // 1) 绮剧‘鍖归厤 slot.name
    let item = slots.find(slot => slot && slot.name === itemName);
    if (item) return item;

    // 2) 鍘绘帀鍛藉悕绌洪棿鍚庡尮閰?(e.g. "twilightforest:knightmetal_chestplate" 鈫?"knightmetal_chestplate")
    const shortName = itemName.includes(':') ? itemName.split(':')[1] : itemName;
    item = slots.find(slot => slot && slot.name === shortName);
    if (item) return item;

    // 3) 甯﹀懡鍚嶇┖闂村尮閰?(鐢ㄦ埛缁?"knightmetal_chestplate"锛宻lot 鍙兘瀛?"twilightforest:knightmetal_chestplate")
    item = slots.find(slot => slot && slot.name && slot.name.endsWith(':' + itemName));
    if (item) return item;

    // 4) Fuzzy match slot.name against the requested item key.
    item = slots.find(slot => slot && slot.name && slot.name.includes(shortName));
    if (item) return item;

    // 5) Match via NBT display name for modded items.
    item = slots.find(slot => {
        if (!slot || !slot.nbt) return false;
        try {
            const display = slot.nbt.value?.display?.value?.Name?.value;
            if (display && display.toLowerCase().includes(shortName.replace(/_/g, ' '))) return true;
        } catch (e) { /* ignore */ }
        return false;
    });
    if (item) return item;

    // 6) 閫氳繃 numeric type ID 鍦?mod_registry 涓煡鎵?    // mod_registry 鍙兘鐭ラ亾杩欎釜 itemName 瀵瑰簲鐨?numeric ID
    try {
        const modReg = mc.getModRegistry?.();
        if (modReg) {
            const fullName = itemName.includes(':') ? itemName : null;
            const regEntry = fullName ? modReg.itemsByName?.get(fullName) : null;
            if (regEntry && regEntry.id !== null) {
                item = slots.find(slot => slot && slot.type === regEntry.id);
                if (item) return item;
            }
        }
    } catch (e) { /* mod_registry not available */ }

    return null;
}

/**
 * 鐢?raw window click 鎶婄墿鍝佺Щ鍒拌澶囨Ы锛堢粫杩?mineflayer 鐨勭墿鍝佺被鍨嬫鏌ワ級
 */
async function _rawEquipToSlot(bot, item, destination) {
    // MC 1.20 player inventory slot mapping:
    ...
    const destSlotMap = {
        ...
        'feet': 8,
        'off-hand': 45,
        'hand': 36, // hotbar slot 0
    };
    const destSlot = destSlotMap[destination];
    if (destSlot === undefined) throw new Error(`Unknown destination: ${destination}`);

    const sourceSlot = item.slot;
    // 浣跨敤 shift-click 鎴?swap锛氬厛鐐规簮妲斤紝鍐嶇偣鐩爣妲?    const window = bot.currentWindow || bot.inventory;
    // 鏂规硶锛氱敤 swap (鎸変綇鐗╁搧 鈫?鏀惧埌鐩爣妲?
    await bot.clickWindow(sourceSlot, 0, 0); // 鎷胯捣鐗╁搧
    await bot.clickWindow(destSlot, 0, 0);   // 鏀惧埌鐩爣妲?    // 濡傛灉鐩爣妲藉師鏉ユ湁鐗╁搧锛岀幇鍦ㄥ湪鍏夋爣涓婏紝鏀惧洖婧愭Ы
    const cursor = bot.inventory.slots[destSlot]; // check if swap happened
    if (bot.heldItem) {
        await bot.clickWindow(sourceSlot, 0, 0); // 鏀惧洖
    }
}
...
function stringifyTrades(bot, trades) {
    return trades.map((trade) => {
        ...
        if (trade.disabled) text += ' x '; else text += ' 禄 ';
...
export async function goToSurface(bot) {
    /**
     * Navigate toward the surface above the bot. First try pathfinding; if that
     * fails underground, dig a bounded upward staircase so repeated calls make
     * steady progress instead of reporting success from deep below.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @returns {Promise<boolean>} true if the surface was reached, false otherwise.
     **/
    const isAirLike = (block) => !block || block.name === 'air' || block.name === 'cave_air' || block.name === 'void_air';
    const isHardStop = (block) => block && (block.name === 'lava' || block.name === 'water' || block.name === 'bedrock' || block.name === 'barrier');
    const targetX = Math.floor(bot.entity.position.x);
    const targetZ = Math.floor(bot.entity.position.z);
    let targetY = null;
    for (let y = 360; y > -64; y--) {
        const block = bot.blockAt(new Vec3(targetX, y, targetZ));
        if (isAirLike(block)) continue;
        targetY = block.position.y + 1;
        break;
    }
    if (targetY == null) return false;

    const startY = Math.floor(bot.entity.position.y);
    log(bot, 'Going to the surface at y=' + targetY + ' from y=' + startY + '; deep underground trips may require multiple progress rounds.');

    const walked = await goToPosition(bot, targetX, targetY, targetZ, 0);
    if (walked && Math.floor(bot.entity.position.y) >= targetY - 1) {
        log(bot, 'Reached surface at y=' + Math.floor(bot.entity.position.y) + '.');
        return true;
    }

    const directions = [[1, 0], [-1, 0], [0, 1], [0, -1]];
    const clearForStep = async (x, y, z) => {
        for (const dy of [0, 1]) {
            const p = new Vec3(x, y + dy, z);
            const block = bot.blockAt(p);
            if (isHardStop(block)) return false;
            if (!isAirLike(block)) {
                const ok = await breakBlockAt(bot, p.x, p.y, p.z);
                if (!ok && !isAirLike(bot.blockAt(p))) return false;
            }
        }
        return true;
    };
    const chooseStep = (pos) => {
        const y = Math.floor(pos.y) + 1;
        const ranked = directions.map(([dx, dz]) => {
            ...
            let score = 0;
            if (isHardStop(feet) || isHardStop(head) || isHardStop(floor)) score -= 100;
            if (isAirLike(feet)) score += 4;
            if (isAirLike(head)) score += 4;
            if (!isAirLike(floor)) score += 2;
            return { dx, dz, x, y, z, score };
        });
        ranked.sort((a, b) => b.score - a.score);
        return ranked[0];
    };

    const climbBudget = Math.min(32, Math.max(0, targetY - Math.floor(bot.entity.position.y)));
    if (climbBudget <= 0) {
        log(bot, 'Surface navigation stopped near y=' + Math.floor(bot.entity.position.y) + '; target was y=' + targetY + '.');
        return false;
    }
    log(bot, 'Path did not reach the surface; digging an upward staircase for up to ' + climbBudget + ' vertical blocks.');
    for (let step = 0; step < climbBudget && Math.floor(bot.entity.position.y) < targetY; step++) {
        if (bot.interrupt_code) return false;
        const next = chooseStep(bot.entity.position);
        if (!next || next.score < -50) {
            log(bot, 'Stopped surfacing: no safe staircase direction from ' + bot.entity.position.floored());
            return false;
        }
        const clear = await clearForStep(next.x, next.y, next.z);
        if (!clear) {
            log(bot, 'Stopped surfacing: blocked or dangerous step at ' + next.x + ',' + next.y + ',' + next.z + '.');
            return false;
        }
        const reachedStep = await goToPosition(bot, next.x, next.y, next.z, 0);
        if (!reachedStep) break;
    }

    const finalY = Math.floor(bot.entity.position.y);
    if (finalY >= targetY - 1) {
        log(bot, 'Reached surface at y=' + finalY + '.');
        return true;
    }
    log(bot, 'Surface progress made/incomplete: started y=' + startY + ', now y=' + finalY + ', target y=' + targetY + ', remaining about ' + Math.max(0, targetY - finalY) + ' blocks. Repeat !goToSurface to continue if safe.');
    return false;
}

function hasInventoryItem(bot, itemName) {
    return Boolean(bot.inventory?.items?.().some(item => item && item.name === itemName));
}

function blockNameAt(bot, pos) {
    try { return bot.blockAt(pos)?.name || 'unknown'; } catch (_) { return 'unknown'; }
}

function isAirLikeBlockName(name) {
...
function isSolidBuildBlock(bot, pos) {
    const block = bot.blockAt(pos);
    if (!block) return false;
    if (isAirLikeBlockName(block.name) || isFluidBlockName(block.name)) return false;
    return block.boundingBox === 'block' || block.diggable;
}

function findSafeStandNear(bot, targetPos) {
    const offsets = [[2,0,0],[-2,0,0],[0,0,2],[0,0,-2],[1,0,1],[1,0,-1],[-1,0,1],[-1,0,-1],[3,0,0],[-3,0,0],[0,0,3],[0,0,-3]];
    const candidates = [];
    for (const item of offsets) {
        const p = targetPos.offset(item[0], item[1], item[2]);
        ...
        if (!isAirLikeBlockName(feet.name) || !isAirLikeBlockName(head.name)) continue;
        if (isAirLikeBlockName(floor.name) || isFluidBlockName(floor.name)) continue;
        candidates.push(p);
    }
...
function findWaterPlacementForLava(bot, lavaPos) {
    const sides = [new Vec3(1,0,0), new Vec3(-1,0,0), new Vec3(0,0,1), new Vec3(0,0,-1), new Vec3(0,1,0)];
    const supports = [new Vec3(1,0,0), new Vec3(-1,0,0), new Vec3(0,0,1), new Vec3(0,0,-1), new Vec3(0,-1,0)];
    for (const side of sides) {
        const waterPos = lavaPos.plus(side);
        const current = blockNameAt(bot, waterPos);
        if (!isAirLikeBlockName(current) && current !== 'lava') continue;
        for (const d of supports) {
            const supportPos = waterPos.plus(d);
            if (supportPos.equals(lavaPos)) continue;
            if (!isSolidBuildBlock(bot, supportPos)) continue;
            const support = bot.blockAt(supportPos);
            return { waterPos, support, faceVec: waterPos.minus(supportPos) };
        }
    }
    return null;
}

async function collectWaterBucket(bot, searchRange=64) {
    if (!hasInventoryItem(bot, 'bucket')) return false;
    const waters = world.getNearestBlocksWhere(bot, block => block.name === 'water' && block.metadata === 0, searchRange, 16)
        .sort((a, b) => bot.entity.position.distanceTo(a.position) - bot.entity.position.distanceTo(b.position));
    for (const water of waters) {
        try {
            const ok = await useToolOnBlock(bot, 'bucket', water);
            if (ok && hasInventoryItem(bot, 'water_bucket')) return true;
        } catch (_) {
            // try another water source
        }
    }
    return hasInventoryItem(bot, 'water_bucket');
}
function findGravelPlacementSpot(bot, radius = 4) {
    ...
    for (const y of [by, by + 1, by - 1]) {
        for (const [dx, dz] of offsets) {
            const pos = new Vec3(bx + dx, y, bz + dz);
            const target = bot.blockAt(pos);
            const below = bot.blockAt(pos.offset(0, -1, 0));
            const head = bot.blockAt(pos.offset(0, 1, 0));
            if (!target || !below || !head) continue;
            if (!isAirLikeBlockName(target.name) || !isAirLikeBlockName(head.name)) continue;
            if (!isSolidBuildBlock(bot, below.position)) continue;
            if (bot.entity.position.distanceTo(pos) < 1.2) continue;
            return pos;
        }
    }
    return null;
...
export async function gatherFlint(bot, targetCount=1, maxAttempts=40) {
    targetCount = Math.max(1, Math.min(8, Number(targetCount) || 1));
    maxAttempts = Math.max(4, Math.min(128, Number(maxAttempts) || 40));
    const flintCount = () => world.getInventoryCounts(bot).flint || 0;
    if (flintCount() >= targetCount) {
        log(bot, 'Already have enough flint: ' + flintCount() + '/' + targetCount + '.');
        return true;
    }
    if (!hasInventoryItem(bot, 'gravel')) {
        log(bot, 'No gravel in inventory; collecting gravel before farming flint.');
        await collectBlock(bot, 'gravel', Math.min(8, maxAttempts));
        await pickupNearbyItems(bot);
        if (flintCount() >= targetCount) return true;
    }
    for (let attempt = 1; attempt <= maxAttempts && flintCount() < targetCount; attempt++) {
        if (!hasInventoryItem(bot, 'gravel')) {
            log(bot, 'No gravel left while farming flint; collecting more gravel.');
            await collectBlock(bot, 'gravel', 8);
            await pickupNearbyItems(bot);
            if (!hasInventoryItem(bot, 'gravel') && flintCount() < targetCount) break;
        }
        const pos = findGravelPlacementSpot(bot, 4);
        if (!pos) {
            log(bot, 'Could not find a safe nearby spot to place gravel for flint farming.');
            return false;
        }
        const placed = await placeBlock(bot, 'gravel', pos.x, pos.y, pos.z, 'bottom', true);
        if (!placed) continue;
        await new Promise(resolve => setTimeout(resolve, 250));
        await breakBlockAt(bot, pos.x, pos.y, pos.z);
        await pickupNearbyItems(bot);
        log(bot, 'Flint farming attempt ' + attempt + '/' + maxAttempts + ': flint=' + flintCount() + '/' + targetCount + '.');
    }
    const total = flintCount();
    log(bot, 'Flint farming stopped with ' + total + '/' + targetCount + ' flint.');
    return total >= targetCount;
}
export async function makeObsidian(bot, targetCount=10, searchRange=64) {
    targetCount = Math.max(1, Math.min(14, Number(targetCount) || 10));
    searchRange = Math.max(16, Math.min(128, Number(searchRange) || 64));
    const obsidianCount = () => world.getInventoryCounts(bot).obsidian || 0;
    const collectAfterMining = async (label) => {
        await new Promise(resolve => setTimeout(resolve, 350));
        await pickupNearbyItems(bot);
        await new Promise(resolve => setTimeout(resolve, 300));
        const total = obsidianCount();
        log(bot, label + ': obsidian inventory is now ' + total + '/' + targetCount + '.');
        return total;
    };
    if (obsidianCount() >= targetCount) {
        log(bot, 'Already have enough obsidian.');
        return true;
    }
    if (!hasInventoryItem(bot, 'diamond_pickaxe') && bot.game?.gameMode !== 'creative') {
        log(bot, 'Need a diamond pickaxe before making and mining obsidian.');
        return false;
    }
        const tried = new Set();
    const protectedPortalPositions = () => {
        const protectedSet = new Set(bot._protectedPortalObsidian || []);
        try {
            const inspection = portals.inspectNetherPortal(bot, searchRange);
            if (inspection.frame) {
                for (const item of inspection.frame.missingFrame || []) {
                    // missing positions are not obsidian, so they do not need mining protection.
                }
                const axis = inspection.frame.axis;
                const vectors = axis === 'z'
                    ? { dx: 0, dz: 1 }
                    : { dx: 1, dz: 0 };
                const offsets = [];
                for (let i = 1; i <= 2; i++) offsets.push([i, 0], [i, 4]);
                for (let h = 1; h <= 3; h++) offsets.push([0, h], [3, h]);
                for (const [i, h] of offsets) {
                    const x = inspection.frame.x + vectors.dx * i;
                    const y = inspection.frame.y + h;
                    const z = inspection.frame.z + vectors.dz * i;
                    protectedSet.add(x + ',' + y + ',' + z);
                    protectedSet.add('(' + x + ', ' + y + ', ' + z + ')');
                }
            }
        } catch (_) {}
    ...
    const isProtectedObsidian = (block) => {
        if (!block?.position) return false;
        const protectedSet = protectedPortalPositions();
        const p = block.position;
        return protectedSet.has(p.toString()) || protectedSet.has(p.x + ',' + p.y + ',' + p.z) || portals.isProtectedPortalObsidian?.(bot, p, searchRange);
    };
    const mineExistingObsidian = async (limitAttempts) => {
        let minedAny = false;
        for (let attempt = 0; attempt < limitAttempts && obsidianCount() < targetCount; attempt++) {
            const obsidianBlocks = world.getNearestBlocksWhere(bot, block => block.name === 'obsidian', searchRange, 64)
                .filter(block => block && !tried.has(block.position.toString()) && !isProtectedObsidian(block))
                .sort((a, b) => bot.entity.position.distanceTo(a.position) - bot.entity.position.distanceTo(b.position));
            if (!obsidianBlocks.length) {
                if (!minedAny) log(bot, 'No existing obsidian found nearby before using bucket/lava method.');
                return minedAny;
            }
            if (attempt === 0) log(bot, 'Found ' + obsidianBlocks.length + ' mineable existing obsidian blocks nearby after protecting any partial portal frame.');
            const block = obsidianBlocks[0];
            tried.add(block.position.toString());
            const mined = await breakBlockAt(bot, block.position.x, block.position.y, block.position.z);
            if (mined) {
                minedAny = true;
                const total = await collectAfterMining('Mined existing obsidian');
                if (total >= targetCount) return true;
            }
            if (bot.interrupt_code) return minedAny;
        }
        return minedAny;
    };

    await mineExistingObsidian(targetCount * 2);
    if (obsidianCount() >= targetCount) return true;

    if (!hasInventoryItem(bot, 'water_bucket')) {
        if (!hasInventoryItem(bot, 'bucket')) {
            log(bot, 'Need a bucket or water_bucket to make more obsidian safely after nearby obsidian was exhausted.');
            return false;
        }
        const gotWater = await collectWaterBucket(bot, searchRange);
        if (!gotWater || !hasInventoryItem(bot, 'water_bucket')) {
            log(bot, 'Could not collect nearby water for obsidian making.');
            return false;
        }
    }
    for (let attempt = 0; attempt < targetCount * 2; attempt++) {
        const minedExisting = await mineExistingObsidian(4);
        if (obsidianCount() >= targetCount) return true;
        if (minedExisting) continue;
        const lavas = world.getNearestBlocksWhere(bot, block => block.name === 'lava' && block.metadata === 0, searchRange, 32)
            .filter(block => block && !tried.has(block.position.toString()))
            .sort((a, b) => bot.entity.position.distanceTo(a.position) - bot.entity.position.distanceTo(b.position));
        if (!lavas.length) {
            log(bot, 'Could not find source lava nearby for obsidian.');
            return (world.getInventoryCounts(bot).obsidian || 0) >= targetCount;
        }
        const lava = lavas[0];
        tried.add(lava.position.toString());
        const stand = findSafeStandNear(bot, lava.position);
        if (stand) await goToPosition(bot, stand.x, stand.y, stand.z, 0.8);
        else await goToPosition(bot, lava.position.x, lava.position.y, lava.position.z, 3);
        const placement = findWaterPlacementForLava(bot, lava.position);
        if (!placement) {
            log(bot, 'No safe water placement found near lava at ' + lava.position + '.');
            continue;
        }
        if (!await equip(bot, 'water_bucket')) return false;
        try {
            await bot.lookAt(placement.support.position.offset(0.5, 0.5, 0.5), true);
            await bot.activateBlock(placement.support, placement.faceVec);
            await new Promise(resolve => setTimeout(resolve, 1600));
        } catch (err) {
            log(bot, 'Failed to pour water near lava: ' + (err.message || err));
            continue;
        }
        const after = bot.blockAt(lava.position);
        if (after && after.name === 'obsidian') {
            const mined = await breakBlockAt(bot, lava.position.x, lava.position.y, lava.position.z);
            if (mined) {
                await collectAfterMining('Made and mined obsidian near ' + lava.position);
            }
        } else {
            log(bot, 'Water did not make obsidian at target lava; now ' + (after?.name || 'unknown') + '.');
        }
        if (!hasInventoryItem(bot, 'water_bucket') && hasInventoryItem(bot, 'bucket')) await collectWaterBucket(bot, searchRange);
        if (obsidianCount() >= targetCount) return true;
    }
    const total = obsidianCount();
    log(bot, 'Obsidian routine stopped with ' + total + '/' + targetCount + ' obsidian.');
    return total >= targetCount;
}
