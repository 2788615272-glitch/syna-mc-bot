...
function entityNameZh(name) {
    if (!name) return '未知实体';
    return ENTITY_NAMES_ZH[name] || name.replaceAll('_', ' ');
}
...
const modes_list = [
    ...
    },
    {
        ...
        update: async function (agent) {
            ...
            if (enemy && await world.isClearPath(agent.bot, enemy)) {
                say(agent, `有危险，附近有 ${entityNameZh(enemy.name)}！`);
                execute(this, agent, async () => {
    ...
    },
    {
        ...
        update: async function (agent) {
            ...
            if (enemy && await world.isClearPath(agent.bot, enemy)) {
                say(agent, `正在和 ${entityNameZh(enemy.name)} 战斗！`);
                execute(this, agent, async () => {
    ...
    },
    {
        ...
        update: async function (agent) {
            ...
            if (huntable && await world.isClearPath(agent.bot, huntable)) {
                execute(this, agent, async () => {
                    say(agent, `正在狩猎 ${entityNameZh(huntable.name)}！`);
...
async function execute(mode, agent, func, timeout=-1) {
    ...
    if (should_reprompt) {
        ...
        agent.handleMessage(role, `[自动系统消息] 你之前的动作“${interrupted_action}”被“${mode.name}”打断。
        行为记录：${logs}\n请根据这个情况回应。`);
    }
}
