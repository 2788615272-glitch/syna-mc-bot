...
function describeServerMotd(description) {
    if (!description) return 'No description provided.';
    if (typeof description === 'string') return description;
    if (typeof description.text === 'string' && description.text.trim()) return description.text;
    if (Array.isArray(description.extra)) {
        const joined = description.extra
    ...
    return new Promise((resolve) => {
        ...
        mc.ping({
            ...
            const serverInfo = {
                host: ip,
                port,
                name: describeServerMotd(response?.description),
...
export async function findServers(ip, earlyExit = false, timeout = 100) {
    ...
    for (let port = startPort; port <= endPort; port++) {
        ...
        if (openPort) {
            const server = await serverInfo(ip, port, 200, false).catch(() => null);
            if (server) {
                servers.push(server);
